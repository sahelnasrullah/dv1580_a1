...
static size_t total_allocated = 0;  // Track total allocated memory across all threads
...
void* mem_alloc(size_t size) 
{
    ...
    if (total_allocated + size > (size_t)memory_pool || memory_left < size) 
    {
        pthread_mutex_unlock(&memory_mutex);
        return NULL;
    }
    ...
    Memory_Block* current = free_memory_list, *prev_block = NULL;
    ...
    while (current != NULL) 
    {
        ...
        if (current_offset >= (size_t)memory_pool) 
        {
            pthread_mutex_unlock(&memory_mutex);
            return NULL;
        }
        ...
        if (current->free && current->size >= size) 
        {
            ...
            if (current_offset + size > (size_t)memory_pool) 
            {
                pthread_mutex_unlock(&memory_mutex);
                return NULL;
            }
    ...
    // Update total_allocated here, after we've confirmed a block was found
    ...
    while (current != NULL) 
    {
        if (current_offset == block_offset) 
        {
            if (!current->free) {  // Only update if block wasn't already free
                ...
                total_allocated -= current->size;  // Decrease total allocated memory
